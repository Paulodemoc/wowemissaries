...
namespace WoWEmissaries.Services
{
  public class WowheadParse : IDisposable
  {
    ...
    {
      //proceed only if connected to the internet OR
      //if settings to use wifi only, proceed only if connected to wifi
      bool wifionly = false;
      ...
      if ((!Plugin.Connectivity.CrossConnectivity.Current.IsConnected) ||
          (wifionly && !Plugin.Connectivity.CrossConnectivity.Current.ConnectionTypes.Contains(Plugin.Connectivity.Abstractions.ConnectionType.WiFi)))
      {
        Device.BeginInvokeOnMainThread(() =>
        {
          MessagingCenter.Send<ActiveEmissariesMessage>(new ActiveEmissariesMessage { ActiveEmissaries = new Dictionary<string, DateTime>() }, "ActiveEmissariesMessage");
        });
      }
      else
      {
